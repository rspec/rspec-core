...
module RSpec::Core::Formatters
  ...
  class Loader
    ...
    def add(formatter_to_use, *paths)
      ...
      if Loader.formatters[formatter_class].nil? && !defined?(RSpec::LegacyFormatters)
        RSpec.warn_deprecation "The #{formatter_class} formatter uses the deprecated formatter interface.\nYou will need to install the `rspec-legacy_formatters` gem to continue using this formatter.\nFormatter added at: #{::RSpec::CallerFilter.first_non_rspec_line}"
      end
  ...
  private
    ...
    def notifications_for(formatter_class)
      formatter_class.ancestors.inject(Set.new) do |notifications, klass|
        notifications + Loader.formatters.fetch(klass) { RSpec::Core::Reporter::NOTIFICATIONS }
      end
    end
