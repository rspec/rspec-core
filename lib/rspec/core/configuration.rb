  ...
  module Core
    ...
    class Configuration
      ...
      def force(hash)
        ...
        [:deprecation_stream, :output_stream].each do |stream|
          hash[stream] = prepare_stream(hash[stream]) if hash.key?(stream)
        end
      ...
      def add_formatter(formatter, output=output_stream)
        output_wrapper.output = prepare_stream(output)
        formatter_loader.add(formatter, output_wrapper)
      end
